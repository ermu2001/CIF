...
class CifNetBottleNeckLayer(nn.Module):
    ...
    def __init__(
        ...
        self.layer = nn.Sequential(
            ...
            CifNetConvLayer(reduces_channels, reduces_channels, stride=stride if not downsample_in_bottleneck else 1),
            CifNetConvLayer(reduces_channels, out_channels, kernel_size=1, activation=None),
        )
        self.activation = ACT2FN[activation]
...
class CifNetStage(nn.Module):
    ...
    def __init__(
        ...
        layer = CifNetBottleNeckLayer if config.layer_type == "bottleneck" else CifNetBasicLayer
...
class CifNetEncoder(nn.Module):
    def __init__(self, config: CifNetConfig):
        ...
        for (in_channels, out_channels), depth in zip(in_out_channels, config.depths[1:]):
            self.stages.append(CifNetStage(config, in_channels, out_channels, depth=depth))
